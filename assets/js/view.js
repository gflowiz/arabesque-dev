...
export default class View{
  ...
  update_import_nodes_modal(nodefeatures){
    ...
    let opts_id = nodefeatures.map(f => {
      if (nodefeatures.indexOf(f) ===0) {return "<option selected>"+f+"</option>"}
      else {return "<option>"+f+"</option>"}
    }).join("");

    let opts_lat = nodefeatures.map(f => {
      if (f.startsWith("lat")) {return "<option selected>"+f+"</option>"}
      else {return "<option>"+f+"</option>"}
    }).join("");

    let opts_long = nodefeatures.map(f => {
      if (f.startsWith("lon")) {return "<option selected>"+f+"</option>"}
      else {return "<option>"+f+"</option>"}
    }).join("");
  ...
  update_import_links_modal(linkfeatures){
    ...
    let opts_origin = linkfeatures.map(f => {
      if (linkfeatures.indexOf(f) ===0) {return "<option selected>"+f+"</option>"}
      else {return "<option>"+f+"</option>"}
    }).join("");

    let opts_dest = linkfeatures.map(f => {
      if (linkfeatures.indexOf(f) ===1) {return "<option selected>"+f+"</option>"}
      else {return "<option>"+f+"</option>"}
    }).join("");

    let opts_volume = linkfeatures.map(f => {
      if (linkfeatures.indexOf(f) ===2) {return "<option selected>"+f+"</option>"}
      else {return "<option>"+f+"</option>"}
    }).join("");
